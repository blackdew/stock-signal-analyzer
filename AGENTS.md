...
이 파일은 Codex가 이 저장소의 코드를 다룰 때 참고하는 가이드입니다.
...
- **수정 전 반드시 실패하는 테스트를 먼저 작성**하여 버그를 재현할 것
- **수정 완료 주장 전 반드시 `uv run pytest`를 실행**하여 검증할 것
- **git history를 먼저 확인**하여 regression 여부를 판단할 것 (`git log`, `git diff`)
