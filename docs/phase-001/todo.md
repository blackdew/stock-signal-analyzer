...
- [ ] `src/indicators/price_levels.py`에 ATR 계산 메서드 추가
  ```python
  def calculate_atr(self, df: pd.DataFrame, period: int = 14) -> pd.Series
  ```
- [ ] High, Low, Close 데이터 확인 및 전처리
- [ ] pandas_ta의 `ta.atr()` 활용
- [ ] NaN 값 처리 로직 추가
- [ ] 단위 테스트 작성
  - 정상 케이스: 충분한 데이터
...
- [ ] 동적 임계값 계산 로직
  ...
  dynamic_knee = floor_price + (current_atr * 2)
  ```
- [ ] 변동성 등급 분류 추가
  - LOW: ATR < 평균의 70%
  - MEDIUM: 평균의 70-130%
...
- [ ] `is_at_shoulder()` 메서드 수정
- [ ] ATR 기반 동적 어깨 임계값 계산
  ```python
  ceiling_price = levels['ceiling']
  dynamic_shoulder = ceiling_price - (current_atr * 2)
  ```
- [ ] 변동성 등급별 조정 적용
- [ ] 단위 테스트 작성
...
- [ ] 새 파일 생성: `src/utils/market_analyzer.py`
- [ ] MarketAnalyzer 클래스 구현
  ...
  class MarketAnalyzer:
      ...
      def calculate_volatility(self) -> str:
          """LOW/MEDIUM/HIGH 반환"""
  ```
- [ ] FinanceDataReader로 KOSPI 데이터 가져오기
- [ ] MA20/MA60 기반 추세 판단 로직
- [ ] 단위 테스트 작성
...
- [ ] `analyze_buy_signals()`에 시장 추세 파라미터 추가
  ```python
  def analyze_buy_signals(
      self,
      df: pd.DataFrame,
      market_trend: str = 'NEUTRAL'  # 추가
  ) -> Dict[str, any]:
  ```
- [ ] 시장 필터 로직 추가
  ```python
...
- [ ] `analyze_sell_signals()`에 시장 추세 파라미터 추가
- [ ] 시장 필터 로직 추가
  ```python
  # 상승장에서는 매도 점수 감점
...
- [ ] `src/analysis/analyzer.py` 수정
- [ ] `__init__`에 MarketAnalyzer 인스턴스 추가
- [ ] `analyze_stock()` 메서드 수정
  ```python
  # 시장 추세 분석
