...
## 분석 요청
위 데이터를 바탕으로 전문 애널리스트 수준의 재무 & 밸류에이션 분석을 작성해주세요.

1. **밸류에이션 매력도**:
   - 현재 PER/PBR이 섹터 평균 및 역사적 추이 대비 저평가/고평가 상태인지 명확히 진단하세요.
   - 단순 수치 비교를 넘어, 이익 성장성(PEG)을 고려한 저평가 여부를 판단하세요.

2. **수익성 및 재무 건전성**:
   - ROE와 영업이익률 추세를 통해 기업의 질적 성장을 평가하세요.
   - 부채비율 등 재무 안정성을 리스크 관점에서 언급하세요.

**작성 가이드**:
- "~함", "~임" 등의 개조식 문체 대신, 전문적인 보고서 문체("~한 것으로 판단됨", "~할 전망임")를 사용하세요.
- 수치를 인용할 때는 괄호를 활용하여 근거를 명확히 하세요.
- **두괄식**으로 핵심 결론을 먼저 제시하세요."""
...
## 분석 요청
위 데이터를 바탕으로 전문적인 기술적 분석 리포트를 작성하세요.

1. **추세 및 패턴**:
   - 현재 주가의 기술적 위치(추세적 상승/하락, 기간 조정 등)를 정의하세요.
   - 의미 있는 차트 패턴(이중바닥, 골든크로스, 수렴 등)이 있다면 언급하세요.

2. **모멘텀 및 시그널**:
   - RSI, MACD 등 보조지표가 시사하는 매수/매도 강도를 해석하세요.
   - 과매수/과매도 구간 진입 여부와 이에 따른 대응 전략을 제시하세요.

3. **가격 전략**:
   - 단기/중기 지지선과 저항선 가격대를 구체적으로 제시하세요.
   - 현 구간에서의 매매 유불리를 판단하세요.

**작성 가이드**:
- 차트의 상황을 눈앞에 그려지듯 묘사하세요.
- 매매 타이밍에 대한 구체적인 힌트를 제공하세요."""
...
## 분석 요청
위 데이터를 바탕으로 시장 심리와 수급 주체들의 의도를 분석하세요.

1. **시장 관심도 및 재료 해석**:
   - [뉴스 헤드라인]을 참고하여 현재 상승/하락의 주된 명분(재료)이 무엇인지 구체적으로 서술하세요. (예: "엔비디아 발 훈풍", "실적 어닝 서프라이즈")
   - 해당 재료의 지속성 여부를 판단하세요.

2. **수급 주체 분석**:
   - 외국인과 기관의 매매 패턴이 의미하는 바를 해석하세요. (매집세, 차익실현, 동반 매수 등)
   - 큰손들의 자금이 유입되고 있는지, 이탈하고 있는지 진단하세요.

**작성 가이드**:
- 뉴스 내용을 적극적으로 인용하여 분석의 구체성을 높이세요.
- 단순히 "수급이 좋다"가 아니라 "외국인의 3일 연속 순매수는 ~를 의미함"과 같이 해석을 덧붙이세요."""
...
## 분석 요청
위 모든 정보를 종합하여 투자자를 위한 **최종 투자 의견 서머리**를 작성하세요.

1. **헤드라인 (한 줄 요약)**:
   - 이 종목을 한마디로 정의하는 매력적이고 강렬한 카피를 작성하세요. (예: "AI 반도체 슈퍼사이클의 최대 수혜주", "조정은 깊었으나 반등은 강력할 것")

2. **투자 포인트 (Investment Thesis)**:
   - [강점]과 루브릭 점수를 바탕으로 이 종목을 매수해야 하는 논리적 근거 3가지를 불렛 포인트로 정리하세요.
   - 각 포인트는 구체적인 근거(데이터, 뉴스, 경쟁력)를 포함해야 합니다.

3. **투자 전략 (Action Plan)**:
   - 현재가 대비 밸류에이션 매력, 기술적 위치, 리스크를 고려하여 구체적인 행동 지침을 주십시오.
   - 보유자 관점(Hold/Sell)과 신규 진입 관점(Buy/Wait)을 나누어 서술하면 좋습니다.

**작성 가이드**:
- 전문 투자 자문사가 고객에게 보내는 프리미엄 리포트 톤앤매너를 유지하세요.
- 투자등급({grade})에 부합하는 논조를 유지하세요. (Strong Buy인 경우 확신에 찬 어조, Hold인 경우 신중한 어조)"""
