"""
Core Configuration Module

섹터 순환 투자 전략을 위한 핵심 설정 파일.
분석 대상 섹터, 루브릭 가중치, 투자 등급 기준 등을 정의합니다.
"""

import os
from typing import Dict, List, Tuple

# =============================================================================
# 분석 대상 섹터 및 대표 종목
# =============================================================================

SECTORS: Dict[str, List[str]] = {
    "반도체": [
        "005930",  # 삼성전자
        "000660",  # SK하이닉스
        "042700",  # 한미반도체
        "403870",  # HPSP
        "058470",  # 리노공업
        "000990",  # DB하이텍
        "240810",  # 원익IPS
        "036930",  # 주성엔지니어링
        "095340",  # ISC
        "039030",  # 이오테크닉스
    ],
    "조선": [
        "010140",  # 삼성중공업
        "009540",  # 한국조선해양
        "042660",  # 한화오션
        "010620",  # 현대미포조선
        "329180",  # HD현대중공업
        "267250",  # HD현대
        "075580",  # 세진중공업
        "033500",  # 동성화인텍
        "023160",  # 태광
        "082740",  # HSD엔진
    ],
    "방산/우주": [
        "012450",  # 한화에어로스페이스
        "047810",  # 한국항공우주
        "064350",  # 현대로템
        "079550",  # LIG넥스원
        "273640",  # 한화시스템
        "103140",  # 풍산
        "298050",  # 쎄트렉아이
        "302920",  # AP위성
        "189300",  # 인텔리안테크
        "274090",  # 켄코아에어로스페이스
    ],
    "전력인프라": [
        "034020",  # 두산에너빌리티
        "267260",  # HD현대일렉트릭
        "298040",  # 효성중공업
        "010120",  # LS ELECTRIC
        "051600",  # 한전KPS
        "052690",  # 한전기술
        "033100",  # 제룡전기
        "001440",  # 대한전선
        "094820",  # 일진파워
        "083650",  # 비에이치아이
    ],
    "바이오": [
        "207940",  # 삼성바이오로직스
        "068270",  # 셀트리온
        "326030",  # SK바이오팜
        "141080",  # 레고켐바이오
        "145020",  # 휴젤
        "000100",  # 유한양행
        "128940",  # 한미약품
        "006280",  # 녹십자
        "185750",  # 종근당
        "069620",  # 대웅제약
    ],
    "로봇": [
        "336260",  # 두산로보틱스
        "108490",  # 로보티즈
        "090710",  # 휴림로봇
        "309930",  # 오토닉스
        "950140",  # 잉글우드랩
        "277810",  # 레인보우로보틱스
        "314930",  # 뉴로메카
        "388790",  # 라온피플
        "049430",  # 코메론
        "090360",  # 로보스타
    ],
    "자동차": [
        "005380",  # 현대자동차
        "000270",  # 기아
        "012330",  # 현대모비스
        "204320",  # 만도
        "161390",  # 한국타이어앤테크놀로지
        "011210",  # 현대위아
        "018880",  # 한온시스템
        "247540",  # 에코프로비엠
        "006400",  # 삼성SDI
        "298050",  # 에스엘
    ],
    "신재생에너지": [
        "009830",  # 한화솔루션
        "112610",  # 씨에스윈드
        "010060",  # OCI홀딩스
        "086520",  # 에코프로
        "003670",  # 포스코퓨처엠
        "096770",  # SK이노베이션
        "058610",  # 에스피지
        "093370",  # 후성
        "389260",  # 대명에너지
        "117580",  # 대성에너지
    ],
    "지주": [
        "000810",  # 삼성화재해상보험
        "003550",  # LG
        "001040",  # CJ
        "005490",  # POSCO홀딩스
        "006400",  # 삼성SDI
        "034730",  # SK
        "000880",  # 한화
        "004990",  # 롯데지주
        "000150",  # 두산
        "078930",  # GS
    ],
    "뷰티": [
        "090430",  # 아모레퍼시픽
        "051900",  # LG생활건강
        "192820",  # 코스맥스
        "161890",  # 한국콜마
        "237880",  # 클리오
        "078520",  # 에이블씨엔씨
        "214420",  # 토니모리
        "002790",  # 아모레G
        "241710",  # 코스메카코리아
        "226320",  # 잇츠한불
    ],
    "금융": [
        "105560",  # KB금융
        "055550",  # 신한지주
        "086790",  # 하나금융지주
        "316140",  # 우리금융지주
        "024110",  # 기업은행
        "032830",  # 삼성생명
        "088350",  # 한화생명
        "138040",  # 메리츠금융지주
        "006800",  # 미래에셋증권
        "039490",  # 키움증권
    ],
    "푸드": [
        "097950",  # CJ제일제당
        "271560",  # 오리온
        "004370",  # 농심
        "280360",  # 롯데웰푸드
        "005300",  # 롯데칠성음료
        "003230",  # 삼양식품
        "005180",  # 빙그레
        "007310",  # 오뚜기
        "000080",  # 하이트진로
        "267980",  # 매일유업
    ],
    "엔터": [
        "352820",  # 하이브
        "041510",  # SM
        "122870",  # YG엔터테인먼트
        "035900",  # JYP엔터테인먼트
        "259960",  # 크래프톤
        "035760",  # CJ ENM
        "253450",  # 스튜디오드래곤
        "263750",  # 펄어비스
        "078340",  # 컴투스
        "251270",  # 넷마블
    ],
}

# =============================================================================
# 루브릭 가중치 (기본값)
# =============================================================================

RUBRIC_WEIGHTS: Dict[str, int] = {
    "technical": 25,          # 기술적 분석 가중치 (추세, RSI, 지지/저항, MACD, ADX)
    "supply": 20,             # 수급 분석 가중치 (외국인, 기관, 거래대금)
    "fundamental": 20,        # 펀더멘털 분석 가중치 (PER, PBR, ROE, 성장률, 부채비율)
    "market": 15,             # 시장 환경 분석 가중치 (뉴스, 섹터모멘텀, 애널리스트)
    "risk": 10,               # 리스크 평가 가중치 (변동성, 베타, 하방리스크)
    "relative_strength": 10,  # 상대 강도 가중치 (섹터내순위, 시장대비알파)
}

# 하위 호환성을 위한 기존 가중치 (V1)
RUBRIC_WEIGHTS_V1: Dict[str, int] = {
    "technical": 30,
    "supply": 25,
    "fundamental": 25,
    "market": 20,
}

# V3: 8대 핵심 루브릭 가중치 (validation-imgs 기준)
# 기존 V2 지표를 재분류하여 8개 카테고리로 확장
RUBRIC_WEIGHTS_V3: Dict[str, int] = {
    "valuation": 20,       # 밸류에이션 (PER, PBR)
    "fundamental": 15,     # 펀더멘털 (ROE, 성장률, 부채비율)
    "supply": 15,          # 수급 (외국인, 기관)
    "momentum": 15,        # 모멘텀 (RSI, MACD, 거래대금)
    "technical": 10,       # 기술적 (추세, 지지/저항, ADX)
    "sector": 10,          # 섹터 (섹터순위, 섹터모멘텀)
    "risk": 10,            # 리스크 (변동성, 베타, 하방리스크)
    "shareholder": 5,      # 주주환원 (배당수익률)
}

# =============================================================================
# 투자 등급 기준
# =============================================================================

INVESTMENT_GRADES: Dict[str, Tuple[int, int]] = {
    "Strong Buy": (80, 100),   # 강력 매수
    "Buy": (60, 79),           # 매수
    "Hold": (40, 59),          # 보유/관망
    "Sell": (20, 39),          # 매도
    "Strong Sell": (0, 19),    # 강력 매도
}

# =============================================================================
# 캐시 설정
# =============================================================================

CACHE_EXPIRE_HOURS: int = 24
CACHE_DIR: str = "output/data/cache"

# =============================================================================
# API 설정
# =============================================================================

# 환경변수에서 API 키 로드
NEWS_API_KEY: str = os.environ.get("NEWS_API_KEY", "")
OPENAI_API_KEY: str = os.environ.get("OPENAI_API_KEY", "")

# =============================================================================
# 유틸리티 함수
# =============================================================================

def get_grade_from_score(score: int) -> str:
    """
    점수를 기반으로 투자 등급을 반환합니다.

    Args:
        score: 0-100 사이의 점수

    Returns:
        투자 등급 문자열 (Strong Buy, Buy, Hold, Sell, Strong Sell)
    """
    for grade, (min_score, max_score) in INVESTMENT_GRADES.items():
        if min_score <= score <= max_score:
            return grade
    return "Hold"  # 기본값


def get_all_sector_symbols() -> List[str]:
    """
    모든 섹터의 종목 코드를 반환합니다.

    Returns:
        전체 종목 코드 리스트 (중복 제거)
    """
    all_symbols = []
    for symbols in SECTORS.values():
        all_symbols.extend(symbols)
    return list(set(all_symbols))


def get_sector_by_symbol(symbol: str) -> str:
    """
    종목 코드로 해당 섹터를 찾습니다.

    Args:
        symbol: 종목 코드

    Returns:
        섹터명 (찾지 못하면 "Unknown")
    """
    for sector, symbols in SECTORS.items():
        if symbol in symbols:
            return sector
    return "Unknown"
